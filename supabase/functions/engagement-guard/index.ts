...
serve(async (req) => {
  try {
    ...
    for (const user of atRiskUsers) {
      ...
      const taskTitle = `Priority Outreach: ${user.email}`;
      ...
      const { error: insertError } = await supabaseAdmin
        ...
        .insert({
          title: taskTitle,
          description: `User engagement has dropped significantly. Health Index is currently ${user.health_index}. Perform manual health check.`,
          priority: 'high',
          status: 'pending',
