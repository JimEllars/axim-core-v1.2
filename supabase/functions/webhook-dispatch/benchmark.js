// Benchmark for Webhook Dispatch
    ...
    for (const webhook of webhooks) {
        try {
            if (webhook.sync_type === 'blob' && fileUrl) {
                const fileResponse = await mockFetch(fileUrl);
                if (!fileResponse.ok) {
                    throw new Error("Failed to fetch file for blob sync");
                }

                const response = await mockFetch(webhook.endpoint_url, {
                    method: "PUT",
                    headers: {
...
async function runBenchmark() {
    ...
    console.log(`Starting benchmark with ${mockWebhooks.length} webhooks...`);
