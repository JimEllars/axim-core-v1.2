...
const TriggerNode = ({ data }) => {
  return (
    <div className="bg-indigo-900/30 border border-cyan-400/50 rounded-lg p-4 shadow-sm min-w-[200px]">
      <div className="flex items-center mb-2">
        <div className="w-8 h-8 rounded-full flex items-center justify-center mr-3 bg-indigo-500/20 text-cyan-400">
...
const ActionNode = ({ data }) => {
  return (
    <div className="glass-effect border border-cyan-500/30 rounded-lg p-4 shadow-sm min-w-[200px]">
...
const WorkflowBuilder = () => {
  ...
  const renderBuilder = () => (
    <div className="h-[600px] border border-cyan-500/30 rounded-lg overflow-hidden glass-effect relative">
      <ReactFlow defaultEdgeOptions={{ style: { stroke: '#06b6d4', strokeWidth: 2 }, animated: true }}
        ...
        fitView
      >
        <Background color="#06b6d4" gap={16} size={1} />
        ...
        <Panel position="top-left" className="flex gap-2">
            <button
              ...
              className="px-3 py-1.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-md text-xs transition-colors flex items-center"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {savedWorkflows.length === 0 ? (
            ...
            savedWorkflows.map((wf, idx) => (
              <div key={wf.id || idx} onClick={() => loadCustomWorkflow(wf)} className="glass-effect border border-cyan-500/30 rounded-lg p-4 hover:border-cyan-400 transition-colors cursor-pointer group">
                <h4 className="text-white font-medium mb-2 group-hover:text-cyan-400">{wf.name}</h4>
                <p className="text-sm text-slate-400 mb-4">{wf.description || 'No description provided.'}</p>
                <button className="text-cyan-400 text-sm font-medium hover:text-indigo-300">Edit Workflow &rarr;</button>
              </div>
            ))
          )}
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            <div key={tpl.id} onClick={() => loadTemplate(tpl.id)} className="glass-effect border border-cyan-500/30 rounded-lg p-4 hover:border-cyan-400 transition-colors cursor-pointer group opacity-60">
               <h4 className="text-white font-medium mb-2 group-hover:text-cyan-400">{tpl.title}</h4>
               <p className="text-sm text-slate-400 mb-4">{tpl.desc}</p>
               <button className="text-cyan-400 text-sm font-medium hover:text-indigo-300">Use Template &rarr;</button>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderSchedule = () => (
    <div className="glass-effect border border-cyan-500/30 rounded-lg p-6">
      ...
      {showScheduleForm && (
        <form onSubmit={handleAddSchedule} className="mb-6 glass-effect p-4 rounded-lg border border-cyan-500/30 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full glass-effect border border-cyan-500/30 rounded px-3 py-2 text-white text-sm focus:border-cyan-400 focus:outline-none"
            />
          </div>
          <div>
      ...
      <div className="space-y-4">
        {scheduledTasks.length === 0 ? (
          ...
          scheduledTasks.map(task => (
            <div key={task.id} className="flex justify-between items-center p-4 glass-effect rounded-md border border-cyan-500/30">
               <div>
        ...
        {activeTab === 'builder' && (
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex items-center px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-medium rounded-md transition-colors shadow-lg shadow-indigo-500/20">
          ...
          { id: 'schedule', label: 'Schedule', icon: FiClock },
           ...
           <button
             ...
             className={`flex items-center px-4 py-2 text-sm font-medium transition-colors border-b-2 ${
               activeTab === tab.id ? 'border-cyan-400 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'
             }`}
           >
